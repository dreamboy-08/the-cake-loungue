...
test.describe('Custom Cake Message & Dynamic Serving Info E2E', () => {
  test('should customize, add to cart, and check order summary', async ({ page }) => {
    // 1. Navigate to the product detail page with bypass=true to skip auth checks if any
    ...
    const oneKgButton = page.locator('button:text("1 Kg")');
    ...
    await page.screenshot({ path: 'custom_cake_checkout_summary.png', fullPage: true });
