...
const DecorationCategoryForm = ({ category, allCategories, onClose, onSuccess }: DecorationCategoryFormProps) => {
    ...
    try {
      ...
      if (imageFile) {
        if (!process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME || process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME === "your_cloud_name") {
          console.warn("Cloudinary not configured, using local blob as preview.");
      ...
      if (!process.env.NEXT_PUBLIC_FIREBASE_API_KEY || process.env.NEXT_PUBLIC_FIREBASE_API_KEY === "your_api_key") {
        console.warn("Firebase not configured, executing local memory category CRUD.");
        const currentList = (window as any)._adminDecorationCategories || [];
        if (category?.id) {
          const updated = currentList.map((c: any) => c.id === category.id ? { ...c, ...categoryData } : c);
          (window as any)._adminDecorationCategories = updated;
        } else {
          const newCat = { ...categoryData, id: 'cat_' + Date.now(), createdAt: new Date().toISOString() };
          (window as any)._adminDecorationCategories = [newCat, ...currentList];
        }
      } else {
        if (category?.id) {
          await updateDoc(doc(db, 'decoration_categories', category.id), categoryData);
        } else {
      ...
      onSuccess(category ? 'Decoration Category updated successfully' : 'Decoration Category created successfully');
      onClose();
    } catch (error) {
      console.error("Error saving decoration category:", error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-chocolate/60 backdrop-blur-sm animate-in fade-in duration-300">
      ...
      >
        <div className="p-8 border-b flex items-center justify-between bg-chocolate text-white shrink-0">
          <div>
            <h2 className="text-2xl font-bold font-playfair">{category ? 'Edit Deco Category' : 'New Deco Category'}</h2>
            <p className="text-[10px] text-white/60 uppercase tracking-widest mt-1">Manage Deco & Essentials Collections</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8 overflow-y-auto">
          <div className="flex flex-col items-center gap-4">
             <div
               ...
               onClick={() => document.getElementById('deco-cat-image')?.click()}
             >
                {imagePreview ? (
                  <>
             ...
             <input id="deco-cat-image" type="file" className="hidden" accept="image/*" onChange={handleImageChange} />
          </div>
          ...
          <div className="space-y-6">
            ...
            <label className="flex items-center gap-3 p-4 rounded-2xl bg-gray-50 border border-gray-100 cursor-pointer group">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-gray-400 uppercase tracking-widest">Visible in shop suggestions & collection page</span>
                </div>
            </label>
          </div>
...
export default DecorationCategoryForm;
